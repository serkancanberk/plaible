...
async function run() {
  ...
  try {
    ...
    const testSession = new UserStorySession({
      ...
      storyPrompt: 'Temporary prompt - will be updated',
      status: 'active',
      currentChapter: 1,
      chaptersGenerated: 0,
    ...
    const generatedPrompt = await generateStoryPrompt(savedTestSession._id.toString());
    if (generatedPrompt) {
    ...
    const testChapter = new Chapter({
      ...
      chapterIndex: 1,
      storyPromptUsed: generatedPrompt || 'Test prompt',
    ...
    const apiGeneratedPrompt = await generateStoryPrompt(savedApiTestSession._id.toString());
    if (apiGeneratedPrompt) {
      savedApiTestSession.storyPrompt = apiGeneratedPrompt;
    ...
    try {
      ...
      const invalidPrompt = await generateStoryPrompt('invalid_session_id');
