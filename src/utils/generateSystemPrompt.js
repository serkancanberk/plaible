...
// Utility function to dynamically generate system prompts for user story sessions
 ...
 */
export async function generateSystemPrompt(sessionId) {
  try {
    // Validate sessionId
    ...
    const basePrompt = story.storyrunner?.systemPrompt;
    if (!basePrompt) {
 ...
 * @returns {Promise<string|null>} - The generated system prompt or null if data is missing
 */
export async function generateSystemPromptForNewSession(storyId, toneStyleId, timeFlavorId) {
  try {
    // Validate inputs
 ...
 */
export async function validateSystemPromptGeneration(storyId, toneStyleId, timeFlavorId) {
  const errors = [];
  ...
  try {
    ...
    } else if (!story.storyrunner?.systemPrompt) {
